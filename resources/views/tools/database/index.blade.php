        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <table class="table table-striped database-tables">
                    <thead>
                        <tr>
                            <th>{{trans('voyager.Table Name')}}</th>
                            <th>BREAD/CRUD {{trans('voyager.Actions')}}</th>
                            <th style="text-align:right">{{trans('voyager.Table Actions')}}</th>
                        </tr>
                    </thead>
                                ...
                                <div class="bread_actions">
                                    @if($table->dataTypeId)
                                        <a class="btn-sm btn-default edit"
                                           href="{{ route('voyager.database.edit_bread', $table->dataTypeId) }}"> {{trans('voyager.Edit')}}
                                            BREAD</a>
                                        <div class="btn-sm btn-danger delete" style="display:inline"
                                             data-id="{{ $table->dataTypeId }}" data-name="{{ $table->name }}"> {{trans('voyager.Delete')}} BREAD
                                        </div>
                                    @else
                                        <form action="{{ route('voyager.database.create_bread') }}" method="POST">
                                            ...
                                            <button type="submit" class="btn-sm btn-default"><i
                                                        class="voyager-plus"></i> {{trans('voyager.Add BREAD to this table')}}
                                            </button>
                                        </form>
                                    @endif
                                </div>
                            ...
                            <td class="actions">
                                <a class="btn-danger btn-sm pull-right delete_table @if($table->dataTypeId) remove-bread-warning @endif"
                                   data-table="{{ $table->name }}" style="display:inline; cursor:pointer;"><i
                                            class="voyager-trash"></i> {{trans('voyager.Delete')}}</a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-outline pull-right" data-dismiss="modal">{{trans('voyager.Cancel')}}</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-trash"></i> {{trans('voyager.Are you sure you want to delete the')}} <span
                                id="delete_table_name"></span> {{trans('voyager.table')}} ? </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.database.destroy', ['database' => '__database']) }}" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="submit" class="btn btn-danger pull-right" value="{{trans('voyager.Yes, delete this table')}}">
                        <button type="button" class="btn btn-outline pull-right" style="margin-right:10px;"
                                data-dismiss="modal">{{trans('voyager.Cancel')}}
                        </button>
                    </form>
    ...
    <div class="modal modal-info fade" tabindex="-1" id="table_info" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body" style="overflow:scroll">
                    <table class="table table-striped">
                        ...
                        <tr>
                            ...
                            <th>{{trans('voyager.Type')}}</th>
                            <th>{{trans('voyager.Null')}}</th>
                            <th>{{trans('voyager.Key')}}</th>
                            <th>{{trans('voyager.Default')}}</th>
                            <th>{{trans('voyager.Extra')}}</th>
                        </tr>
                        </thead>
                        <tbody>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline pull-right" data-dismiss="modal">{{trans('voyager.Close')}}</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
