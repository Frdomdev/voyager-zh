...
@section('page_header')
    <h1 class="page-title">
        ...
        @if(isset($table))
            {{trans('voyager.Editing')}}{{ $table }} {{trans('voyager.table')}}
        @else
...
@section('content')
    ...
    <table class="table table-bordered" style="width:100%; display:none;">
        ...
        <tr>
            <th></th>
            <th>{{trans('voyager.Field Name')}}</th>
            <th>{{trans('voyager.DB Type')}}</th>
            <th>{{trans('voyager.Allow Null?')}}</th>
            <th>{{trans('voyager.Key')}}</th>
            <th>{{trans('voyager.Default Value')}}</th>
            <th></th>
        </tr>
        </thead>
        <tbody>
                ...
                <form action="{{ $formAction }}" method="POST">
                    ...
                    <div class="panel panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title">@if(isset($table)) {{trans('voyager.Edit the')}} {{$table}} {{trans('voyager.table below')}} @else {{trans('voyager.Create Your New Table Below') }}@endif</h3>
                        </div>
                        ...
                        <div class="panel-body">
                            ...
                            @else
                                <div class="col-md-6">
                            @endif
                                    <label for="name">{{trans('voyager.Table Name')}}</label><br>
                                ...
                                @if(!isset($table))
                                    <div class="col-md-3 col-sm-4 col-xs-6">
                                        <label for="create_model">{{trans('voyager.Create model for this table?')}}</label><br>
                                        <input type="checkbox" name="create_model" data-toggle="toggle"
                                               data-on="{{trans('voyager.Yes, Please')}}" data-off="{{trans('voyager.No Thanks')}}">
                                    </div>

                                    <div class="col-md-3 col-sm-4 col-xs-6">
                                        <label for="create_migration">{{trans('voyager.Create migration for this table?')}}</label><br>
                                        <input type="checkbox" name="create_migration" data-toggle="toggle"
                                               data-on="{{trans('voyager.Yes, Please')}}" data-off="{{trans('voyager.No Thanks')}}">
                                    </div>
                                @endif
                            </div><!-- .panel-body .row -->
                            <p>{{trans('voyager.Table Fields')}}</p>
                            ...
                            <div style="text-align:center">
                                <div class="btn btn-success" id="newField">{{trans('voyager.+ Add New Field')}}</div>
                                <div class="btn btn-success" id="newFieldPrimary">{{trans('voyager.+ Add Primary Field')}}</div>
                                @if(!isset($table))
                                    <div class="btn btn-success" id="newFieldTimestamps">{{trans('voyager.+ Add Timestamp Fields')}}</div>
                                    <div class="btn btn-success" id="newFieldSoftDelete">{{trans('voyager.+ Add Soft Delete Field')}}</div>
                                @endif
                            </div>
                        </div><!-- .panel-body -->
                        ...
                        <div class="panel-footer">
                            <input type="submit" class="btn btn-primary pull-right"
                                   value="@if(isset($table)){{trans('voyager.Update Table') }}@else{{ trans('voyager.Create New Table') }}@endif">
    ...
    <script>
        ...
        function newRow(kind, name, type, nullable, key, defaultValue) {
            ...
            $('.toggleswitch').not('.tablerow .toggleswitch').bootstrapToggle({
                on: '{{trans("voyager.Yes")}}',
                off: '{{trans("voyager.No")}}'
            });
