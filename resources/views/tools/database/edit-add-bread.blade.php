...
@section('page_header')
    <div class="page-title">
        <i class="voyager-data"></i> @if(isset($dataType->id)){{trans('voyager.Edit BREAD for')}}  {{$dataType->name}} {{trans('voyager.table')}} @elseif(isset($table)){{ 'Create BREAD for ' . $table . ' table' }}@endif
    </div>
@stop
                ...
                <form role="form"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title">{{trans('voyager.Edit the rows for the')}} {{ $table }} {{trans('voyager.table below')}}:</h3>
                        </div>

                        <table id="users" class="table table-hover">
                            <thead>
                            <tr>
                                <th>{{trans('voyager.Field')}}</th>
                                <th>{{trans('voyager.Field Info')}}</th>
                                <th>{{trans('voyager.Visibility')}}</th>
                                <th>{{trans('voyager.Input Type')}}</th>
                                <th>{{trans('voyager.Display Name')}}</th>
                                <th>{{trans('voyager.Optional Details')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($fieldOptions as $data)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <strong>{{trans('voyager.Type')}}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{trans('voyager.Key')}}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{trans('voyager.Required')}}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{trans('voyager.Yes')}}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}"
                                                   checked="checked">
                                        @else
                                            <span>{{trans('voyager.No')}}</span>
                                    ...
                                    <td>
                                        ...
                                        <label for="field_browse_{{ $data['field'] }}">{{trans('voyager.Browse')}}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{trans('voyager.Read')}}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{trans('voyager.Edit')}}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{trans('voyager.Add')}}</label><br/>
                                        <input type="checkbox"
                                               ...
                                               name="field_delete_{{ $data['field'] }}" @if(isset($dataRow->delete) && $dataRow->delete){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['key'] == 'timestamp' && $data['field'] == 'updated_at')@elseif(!isset($dataRow->delete)){{ 'checked="checked"' }}@endif>
                                                <label for="field_delete_{{ $data['field'] }}">{{trans('voyager.Delete')}}</label><br/>
                                    </td>
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ ucfirst($table) }} BREAD {{trans('voyager.info')}}</h3>
                        </div>

                        <div class="panel-body">
                            <div class="form-group">
                                <label for="name">{{trans('voyager.Table Name')}}</label>
                            ...
                            <div class="form-group">
                                <label for="email">{{trans('voyager.URL Slug (must be unique)')}}</label>
                            ...
                            <div class="form-group">
                                <label for="email">{{trans('voyager.Display Name (Singular)')}}</label>
                            ...
                            <div class="form-group">
                                <label for="email">{{trans('voyager.Display Name (Plural)')}}</label>
                            ...
                            <div class="form-group">
                                <label for="email">{{trans('voyager.Icon (optional) Use a')}} <a
                            ...
                            <div class="form-group">
                                <label for="email">{{trans('voyager.Model Name')}} (ex. \App\User, {{trans('voyager.if left empty will try and use the table name')}} )</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="generate_permissions">{{trans('voyager.Generate Permissions')}}</label><br>
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="server_side">{{trans('voyager.Server-side Pagination')}}</label><br>
                            ...
                            <div class="form-group">
                                <label for="description">{{trans('voyager.Description')}}</label>
                                <textarea class="form-control" name="description"
                                          placeholder="Description">@if(isset($dataType->description)){{ $dataType->description }}@endif</textarea>
                            </div>

                            <div class="box-footer">
                                <button type="submit" class="btn btn-primary">{{trans('voyager.Submit')}}</button>
                            </div>
                        </div><!-- .panel-body -->
