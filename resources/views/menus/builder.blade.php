            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered">
                    ...
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">{{trans('voyager.Drag and drop the menu Items below to re-arrange them')}}.</p>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-plus"></i> {{trans('voyager.Create a New Menu Item')}}</h4>
                </div>
                <form action="{{ route('voyager.menus.item.add', ['menu' => $menu->id]) }}" id="delete_form" method="POST">
                    <div class="modal-body">
                        <label for="name">{{trans('voyager.Title of the Menu Item')}}</label>
                        <input type="text" class="form-control" name="title" placeholder="Title"><br>
                        <label for="url">{{trans('voyager.URL for the Menu Item')}}</label>
                        ...
                        <label for="color">{{trans('voyager.Color in RGB or hex (optional)')}}</label>
                        <input type="color" class="form-control" name="color"
                               placeholder="Color (ex. #ffffff or rgb(255, 255, 255)"><br>
                        <label for="target">{{trans('voyager.Open In')}}</label>
                        <select id="edit_target" class="form-control" name="target">
                            <option value="_self">{{trans('voyager.Same Tab/Window')}}</option>
                            <option value="_blank">{{trans('voyager.New Tab/Window')}}</option>
                        </select>
                    ...
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm" value="{{trans('voyager.Add New Item')}}">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{trans('voyager.Cancel')}}</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-edit"></i> {{trans('voyager.Edit Menu Item')}}</h4>
                </div>
                <form action="{{ route('voyager.menus.item.update', ['menu' => $menu->id]) }}" id="edit_form" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <label for="target">{{trans('voyager.Open In')}}</label>
                        <select id="edit_target" class="form-control" name="target">
                            <option value="_self" selected="selected">{{trans('voyager.Same Tab/Window')}}</option>
                            <option value="_blank">{{trans('voyager.New Tab/Window')}}</option>
                        </select>
                    ...
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm" value="{{trans('voyager.Update')}}">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{trans('voyager.Cancel')}}</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
